            ...
            <div class="w-full  text-cyan-800 flex">
                ...
                <div class="w-5/6 my-32 py-12 px-32 mx-32 flex flex-col bg-white rounded-2xl shadow-2xl">
                    ...
                    <div class="my-4">
                        ...
                        <input type="text" class="border rounded-lg mt-3 pl-3 w-full h-12 bg-white" v-model.trim="user.username">
                        <div class="text-lg text-black mt-4">Name</div>
                        <input type="text" class="border rounded-lg mt-3 pl-3 w-full h-12 bg-white" v-model.trim="user.name" maxlength="100">
                        <div class="text-lg text-black mt-4">Email</div>
                        <input type="email" class="border rounded-lg mt-3 pl-3 w-full h-12 bg-white" v-model.trim="user.email" maxlength="150">
                        <div class="text-lg text-black mt-4">Role</div>
                        <select class="select select-bordered bg-white mt-3" v-model="user.role">
                            <option>admin</option>
                        ...
                        <div class="flex mt-4" v-if="!isAddUserPage">
                            <div class="mr-10">
                                ...
                                {{ formatDatetimeLocal(user.createdOn) }}
                            </div>
                            <div>
                                <span class="font-bold mr-3">Updated On</span>
                                {{ formatDatetimeLocal(user.updatedOn) }}
                            </div>
                        </div>
                        <div class="flex mt-4">
                            <button 
                                class="text-white bg-emerald-plus hover:bg-emerald-light border-0 shadow-lg hover:scale-110 w-28 h-12 rounded-lg">
                                Save
                            </button>
                            <button 
