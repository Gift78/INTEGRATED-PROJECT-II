...
const matchPassword = async () => {
    ...
    if (response.ok) {
        ...
        if (errorData.message.slice(0, 18) === 'User with username') {
                ...
                <div class="w-3/5 my-32 py-12 px-32 mx-32 flex flex-col bg-white rounded-2xl shadow-2xl ">
                    ...
                    <div v-if="matchMsg !== ''">
                        <div v-if="matchMsg === 'Password Matched'"
                            class="mt-10 border-2 p-5 border-emerald-500 bg-emerald-50 rounded-xl text-emerald-800">
                            {{ matchMsg }}
                        </div>
                        <div v-if="matchMsg === 'The specified username DOES NOT exist'"
                            class="mt-10 border-2 p-5 border-red-500 bg-red-50 rounded-xl text-red-800">
                            {{ matchMsg }}
                        </div>
                    ...
                    <input type="password" class="ann-username border rounded-lg mt-3 pl-3 w-full h-12 bg-white"
                        v-model="password">
